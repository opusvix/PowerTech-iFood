...
select 
	...
	po.Preco_unitario, 
	round(po.Preco_unitario*pp.Quantidade_produto,2) as valor_da_compra, 
	pe.Status_do_pedido, 
	cd.Numero_carteira, 
	cd.Valor_em_conta
from cliente c
...
select 
	...
	pe.Quantidade_estoque, 
	p.Preco_unitario, 
	round(p.Preco_unitario*Quantidade_estoque,2) as Valor_estoque 
from produto p
inner join produto_em_estoque pe on idProduto=idEsProduto
...
select 
	sum(round(p.Preco_unitario*Quantidade_estoque,2)) as Valor_estoque 
from produto p
inner join produto_em_estoque pe on idProduto=idEsProduto
